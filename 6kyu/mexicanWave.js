...
// even though individual spectators never move away from their seats. In many large arenas the
// crowd is seated in a contiguous circuit all the way around the sport field, and so the wave
...
function wave(str){
    ...
    for (let i = 0; i < len; i++) {
        if(str[i]===' ')continue;
        wave.push(str.slice(0, i).concat(str[i].toUpperCase(), str.slice(i+1, len)));
    }
    return wave;
}
module.exports = wave;